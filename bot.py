...
#A function to get the City name
def getCityName(text):
    words= text.split() #Spliting the text into a list of words
    for i in range(0, len(words)):
        #A Check to Compare the first word with 'who' and the next word with 'is'.
        #If true, return the person's first name and person's last name
        if i+3 <= len(words)-1 and words[i].lower()=='where' and words[i+1].lower()=='is':
            #Returning the person's first and last name
            return words[i+2]+ ' ' + words[i+3]
...
while True:
    ...
    if(wakeWord(text)==True):
        ...
        if('how are you' in text):
            response= response+ ' Its been a good day. Thanks for asking!'

        if ('who are you' in text or 'what is your name' in text):
            response = response + ' My name is Alpha. I am your personal assistant.' \
                                  'I am here to make your life easier. You can command me to perform' \
                                  'various tasks which you want to'

        if ('who has created you' in text or 'who made you' in text):
            response = response + ' I am created by amir khan who is a student of ' \
                                  'Lovely Professional University, Jalandhar, Punjab'


        if('where is' in text):
            city = getCityName(text)
            wiki = wikipedia.summary(city, sentences=2)
            response = response + ' ' + wiki
